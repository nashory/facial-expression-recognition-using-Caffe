...
ALL_DATA_ROOT=$PROJECT_PATH/database/preprocessed/$TARGET/all/
ALL_LMDB_OUTPUT=$LMDB_OUTPUT/all_lmdb
...
echo "Creating all lmdb..."
GLOG_logtostderr=1 $TOOLS/convert_imageset \
    --resize_height=$RESIZE_HEIGHT \
    --resize_width=$RESIZE_WIDTH \
    --shuffle \
    $ALL_DATA_ROOT \
    $DB_LIST/all/db_list.txt \
    $ALL_LMDB_OUTPUT
echo "Creating all lmdb mean proto..."
$TOOLS/compute_image_mean $ALL_LMDB_OUTPUT \
  $ALL_LMDB_OUTPUT/mean.binaryproto
